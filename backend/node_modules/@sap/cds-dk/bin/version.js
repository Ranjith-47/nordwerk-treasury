...
module.exports = exports = async function cds_version (pattern, options) {
  ...
  if (!o.json) {
    ...
    if (o._elsp) {
      if (!cds.utils.fs.existsSync(join(o.root, 'node_modules')))
        console.log (`\nNo dependencies installed. Run ${BOLD}npm install${RESET}, then rerun ${BOLD}cds version${RESET}.`)
      else
...
const $ = cds.utils.sh || sh_polyfill
...
async function node_versions(o) {
  ...
  const [ [npm_root_g],[npm_root_l], lines ] = await Promise.all ([
    $`npm root --global`,
    $`npm root --local`,
    $`npm ls -lp ${
      ...
      const isELSPROBLEMS = /\bELSPROBLEMS\b/.test(err)
      if (isELSPROBLEMS) {
        o._elsp = true
        return (e.stdout || '').trim().split('\n').filter(Boolean)
      }
      const code = e.code ?? e.signal ?? 'UNKNOWN'
      const cmd = `npm ls -lp ${(o.omit ? o.omit.split(',').map(o => `--omit=${o} `).join('') : '') + (o.depth ? `--depth=${o.depth}` : '--depth=1')}`
      const msg =
...
function java_versions (o) {
  ...
  if (!javaInfo) return []
  else if (process.stderr.isTTY && !o.json) console.warn (DIMMED, 'This takes a while. Collecting Java versions...\n', RESET)
...
// Polyfill for cds.utils.sh, which is only available in cds>=9.8.0.
// cds-version stays extra-compatible to allow listing and troubleshooting older versions.
/**
 ...
 */
function sh_polyfill (cmd,..._) {
  if (cmd.raw) cmd = String.raw (cmd,..._) // the cmd may be a tagged template
  let debug = sh_polyfill.debug ??= cds.debug('shell'); debug?.(cmd)
  let exec = sh_polyfill.exec ??= require('node:child_process').exec
