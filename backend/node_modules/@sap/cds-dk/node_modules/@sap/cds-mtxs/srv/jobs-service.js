...
module.exports = class JobsService extends cds.ApplicationService {
  ...
  async init() {
    ...
    this.jobQueue = new Queue(this._conf.queueSize)
    ...
    this.on('config', () => {
      const conf = main.requires['cds.xt.ExtensibilityService'], ext = main.requires.extensibility
      ...
      return {
        ...
        extensibility: {
          tenantCheckInterval: conf?.tenantCheckInterval ?? ext?.tenantCheckInterval,
          [allowlist]: conf?.[allowlist] ?? ext?.[allowlist],
          [blocklist]: conf?.[blocklist] ?? ext?.[blocklist]
        }
      }
    })
  ...
  _config() {
    ...
    return { queueSize, clusterSize, workerSize, poolSize }
  }
  ...
  async enqueue(service, op, clusters, args, onJobDone) {
    ...
    if (tasks.length) {
      if (cds.services[service]) {
        ...
        this.jobQueue.enqueue({ job_ID, clusters: clustered, fn, onJobDone })
      } else {
        this.jobQueue.enqueue({ job_ID, clusters: clustered, fn: () => {
  ...
  async pickJob() {
    ...
    const next = new Set(this.jobQueue.peek().clusters.flat().map(t => t.tenant).flat())
    const running = [...this.runningJobs.values()].flatMap(j => j.clusters.flat().map(t => t.tenant))
    if (running.some(t => next.has(t))) return

    const job = this.jobQueue.dequeue()
    const { job_ID, clusters, fn, onJobDone } = job
    try {
      this.runningJobs.set(job_ID, job)
      await t0_(async () => {
  ...
  async _nextJob(clusters, fn, onJobDone) {
    if (clusters.length > 1) {
      await this.limiter(this._conf.clusterSize, clusters, cluster => this.limiter(this._conf.workerSize ?? this._conf.poolSize, Array.from(cluster), fn, true))
    } else {
      await this.limiter(this._conf.workerSize ?? this._conf.poolSize, Array.from(clusters[0]), fn, true)
    }
