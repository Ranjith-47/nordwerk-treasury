  ...
  return async function $metadata(req, res, next) {
    ...
    if (mps) {
      ...
      try {
        ...
        if (!edmx) {
          // IMPORTANT: Only provide the extended model!
          // If model is undefined, MPS has a shortcut to return the prebuilt edmx
          edmx = await mps.getEdmx({ tenant, model: cds.context.model, service: service.definition.name })
