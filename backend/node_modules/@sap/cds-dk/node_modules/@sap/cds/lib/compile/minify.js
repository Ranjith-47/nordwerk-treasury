...
class Minifier {
  ...
  minify (csn, options, { skip_unused } = global.cds.env.features) {
    ...
    const keep = (d,kinds) => d.kind in kinds || _can_minify(d)
    ...
    if (o.services) {
      ...
      for (let [s,d] of all) if (d.kind === 'service' && _skip_service(s,d)) {
        skipped[s] = 0; children (s, (c,d) => keep(d,_ext) ? this.keep(c,d,s) : skipped[c] = s) // used later on in this.keep()
      }
      // Then keep all own services and their children
      for (let [s,d] of all) if (d.kind === 'service' && !(s in skipped)) {
        this.keep(s,d); children (s, (c,d) => keep(d,_own) ? this.keep(c,d) : skipped[c] = 0)
      }
      // Also keep remaining non-service entities
      for (let [e,d] of all) if (d.kind === 'entity' || !_can_minify(d)) {
...
const _skip_service = (s,d) => (d['@cds.external'] >= 2) && _can_minify(d)
const _skip_entity = (e,d) => (d['@cds.external'] >= 2 || d['@cds.persistence.skip'] === 'if-unused') && _can_minify(d) || e.endsWith('.texts')
const _can_minify = d => d['@cds.minify'] !== false
