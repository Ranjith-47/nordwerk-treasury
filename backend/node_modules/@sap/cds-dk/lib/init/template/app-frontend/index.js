...
module.exports = class extends require('../../plugin') {
    ...
    async run() {
        ...
        for (const { app } of apps) {
            ...
            if (ui5) {
                ...
                if (project.isTypescript) additions.unshift({ in: 'builder.customTasks', where: { name: 'ui5-tooling-transpile-task' }, at: 0 })
    ...
    async combine() {
        ...
        const {
            ...
            addVue, addReact, addAppFrontend, hasKyma, addKyma
        } = project
        ...
        if (addMta || affected && hasMta) {
            ...
            await merge(__dirname, 'files/mta.yaml.hbs').into('mta.yaml', {
                with: project,
                additions: [srvModule, appDeployer, appFront, ...appModules],
                relationships
            })
        }
        ...
        if (addAppFrontend || addVue || addReact) {
            ...
            const isRootLayout = addVite && (into === undefined || into === '' || into === '.')
            // Candidate paths to set up: each sub-folder app, plus optionally the app/ root for the single-UI layout
            const candidates = apps.map(({ app }) => ({ app, appPath: join(cds.root, cds.env.folders.app, app), isRoot: false }))
            const rootAppPath = join(cds.root, cds.env.folders.app)
            if (exists(join(rootAppPath, 'package.json'))) {
                candidates.push({ app: '', appPath: rootAppPath, isRoot: true })
            }
            for (const { app, appPath, isRoot } of candidates) {
                if (!exists(join(appPath, 'package.json'))) continue
                const pJson = await read(join(appPath, 'package.json'))
                if (!pJson.devDependencies?.vite) continue
                if (addVite && !isRootLayout && into !== app) continue
                if (addVite && isRootLayout && !isRoot) continue
