...
function dataForField(element, artifact, csn, record, data, parent, depth, referenceData) {
  ...
  switch (type) {
    ...
    case 'cds.Composition': {
      ...
      if (ownKey && targetKey) {
        // carry over all parent keys, not just the first, so composite/nested keys survive
        const value = ownKeyStructured
          ? Object.fromEntries(Object.keys(artifact.keys).map(k => [k, record[k]]))
          : record[ownKey]
