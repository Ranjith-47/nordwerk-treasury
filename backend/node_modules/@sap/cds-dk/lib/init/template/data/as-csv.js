      ...
      for (let i = 0; i < entityData.length; i++) {
        ...
        for (const element of elements) {
          ...
          if (element._type === 'cds.Composition') {
            ...
            // flatten nested foreign keys (composite keys of nested aspects) into up__up__ID columns
            const {headers: fkHeaders, records: fkRecords} = resolveStruct(value, name)
            if (i === 0) {
              header.push(...fkHeaders)
            }
            record.push(...fkRecords)
          }
