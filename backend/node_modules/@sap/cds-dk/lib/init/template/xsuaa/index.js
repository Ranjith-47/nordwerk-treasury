...
module.exports = class XsuaaTemplate extends require('../../plugin') {
    ...
    async combine() {
        const project = readProject()
        const { addMultitenancy, hasMultitenancy, addMta, hasMta, addKyma, isJava, srv, hasKyma } = project
        ...
        if (addKyma || addMultitenancy && hasKyma) {
            ...
            if (!project.hasCustomRoleCollections && xsSecurity['role-templates'].length) {
                parameters['role-collections'] = xsSecurity['role-templates'].map(role => ({
                    name: `${role.name} (${project.appName} {{ .Release.Namespace }})`,
                    description: 'generated',
                    'role-template-references': [`$XSAPPNAME.${role.name}`],
                }))
            }
            await merge({ xsuaa: { parameters } }).into('chart/values.yaml')
