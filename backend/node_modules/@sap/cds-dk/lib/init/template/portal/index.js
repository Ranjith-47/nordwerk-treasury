...
module.exports = class PortalTemplate extends require('../../plugin') {
  ...
  async combine() {
    const project = readProject()
    const { isJava, addMta, hasMta, srv, addHtml5Repo, addXsuaa, addApprouter, addMultitenancy, addKyma, hasApprouter, hasHtml5Repo, hasMultitenancy, hasXsuaa } = project
    ...
    if (addMta || (addXsuaa || addHtml5Repo || addApprouter || addMultitenancy) && hasMta) {
      ...
      const additions = [srvModule, portal, appDeployer, portalDeployer]
      const relationships = [{
        insert: [portal, 'name'],
    ...
    if (addKyma) {
      ...
      await renderAndCopy(path.join(__dirname, 'files/portal-site'), 'app/html5-deployer/portal/portal-site', project)
    }
  }
}
