...
function createQuery(client, refs) {
  ...
  if (Object.prototype.toString.call(refs) === '[object Array]') {
    ...
    for (const ref in refs) {
      const proj = refs[ref].projection;
...
function errorQuery(error) {
  ...
  q.$execute = function(client, config, callback) {
    const params = AbstractQuery
        .Private.normalizeExecuteCallParams(this, client, config, callback);
...
AbstractQuery.prototype = function() {
  function _addOperation(option, allowedOptions) {
    let result = '';
    for (let i = 0; result.length === 0 & i < allowedOptions.length; i++) {
  ...
  function union(query, option) {
    return new SetQuery(this.qconn,
        this,
        ` UNION ${ _addOperation(option, ['$all', '$distinct'])}`,
        query,
        this.entities,
        this.leadingAlias,
        this.aliasSynonyms);
  ...
  function intersect(query, option) {
    return new SetQuery(this.qconn,
        this,
        ` INTERSECT ${ _addOperation(option, ['$distinct'])}`,
        query,
        this.entities,
        this.leadingAlias,
        this.aliasSynonyms);
  ...
  function except(query, option) {
    return new SetQuery(this.qconn,
        this,
        ` EXCEPT ${ _addOperation(option, ['$distinct'])}`,
        query,
        this.entities,
        this.leadingAlias,
        this.aliasSynonyms);
  ...
  return {
    ...
    /**
         * Unites two queries. The queries must have the same number of columns.
         * Additionally the columns must have the same types.
         *
         * @param {boolean} [option.$all] if true, selects all records from all select statements.
         *                                Duplicates are not removed.
         * @param {boolean} [option.$distinct] if true, selects all unique records from all
         *                                     select statements by removing duplicates found in
    ...
    /**
         ...
         * Additionally the columns must have the same types.
         *
         * @param {boolean} [option.$distinct] if true, returns records that exist in
         *                                     all select statement results (after $execute())
         * @function
         * @memberof cds.Query.prototype
    ...
    /**
         *  Removes the entries from the second query from the first.
         *  The queries must have the same number of columns.
         *  Additionally the columns must have the same types.
         *
         * @param {boolean} [option.$distinct] if true, returns all unique records from the first
         *                                     select statements after removing the duplicates in
         *                                     the following select statements (after $execute()).
         * @function
         * @memberof cds.Query.prototype
...
AbstractQuery.Private = {
  ...
  /* adds joins to the entities, using the name assocName.
     ...
     * curEntity is the entity which is the source for the navigation
     * (or the target if boolean flag "reverse" is set)
     * alias is the column alias used for referencing the target
     * prevAlias is the column alias used for referencing the source
     *
     */
  addAssociationJoins:
    function(entities, assocName, association, curEntity, alias, prevAlias, reverse) {
      ...
      function flat(keys, prefix) {
        ...
        for (const key in keys) {
          ...
          if (keys[key].$column) {
            ...
            const recursive = flat(keys[key], newPrefix);
            for (const r in recursive) {
              res[r] = recursive[r];
            }
          }
        }
      ...
      if (association.$association.$viaEntity) {
        const viaEntity = association.$association.$viaClass;
        const viaAlias = `${alias }$viaEntity`;
        AbstractQuery
            .Private
            .addAssociationJoins(entities, viaAlias,
                viaEntity.$_mapping[association.$association.$source],
                viaEntity, prevAlias, viaAlias, true);
        AbstractQuery
            .Private
            .addAssociationJoins(entities, assocName,
                viaEntity.$_mapping[association.$association.$target],
                viaEntity, assocName, viaAlias);
      } else if (association.$association.$on) {
        const assocRef = assocName.substring(assocName.lastIndexOf('.') + 1);
        const associatedEntity = association.$association.$class;
        entities[assocName] = {
          entity: associatedEntity,
        };
        let j = association.$association.$on;
        const matches = j.match(/\$SOURCE(\.\"\w+\")+/g);
        if (matches) {
        ...
        const backlink = association.$association.$viaBacklink;
        let fks = backlink ? associatedEntity.$_mapping[backlink] : association;
        fks = flat(fks);
        for (const fk in fks) {
          if (fk.indexOf('$') === 0) {
            continue;
          ...
          if (backlink) {
            j = {
              _eq: [{
                _col: [
                  curEntity,
                  replaceAssocAliases(fk, association.$association),
                  prevAlias,
                ],
              }, {
  ...
  serializeExpression: function(expr, entityset) {
    ...
    if (Object.prototype.toString.call(expr) === '[object Array]') {
      return expr.map(function(e) {
        const str = AbstractQuery.Private.serializeExpression(e, entityset);
        return str;
      });
    }
    let notat = expr && expr.notation ? expr.notation : null;
    ...
    if (notat) {
      for (const prop in expr) {
        if (Object.prototype.toString.call(expr[prop]) === '[object Array]') {
          const str = AbstractQuery
              .Private
              .serializeExpression(expr[prop], entityset)
              .concat(
                  [entityset],
              );
          return notat(str);
  ...
  _addJoinEntities: function(entities, cond, alias, firstLevel) {
    for (const key in cond) {
      if (key === '_col') {
        alias = AbstractQuery
            .Private
            ._addJoinEntities(entities,
                cond[key][2]._col ? cond[key][2] : undefined,
                alias);
        const curEntity = cond[key][0];
        const mapping = curEntity.$_mapping ? curEntity.$_mapping : curEntity;
        if (mapping[cond[key][1]] && mapping[cond[key][1]].$association) {
          AbstractQuery
              .Private
              .addAssociationJoins(entities,
                  `${alias }.${ cond[key][1]}`, mapping[cond[key][1]],
                  curEntity,
                  `${alias }.${ cond[key][1]}`,
                  alias,
              );
  ...
  transformResult: function(d, metadata) {
    switch (metadata.dataType) {
      case 15: // TIME
        const REGEX_TIME = /(\d{2}):(\d{2}):(\d{2})(\.(\d+))?/;
        const m = d.match(REGEX_TIME);
        if (m) {
  ...
  recordToObject: function(record, query, config, metadata) {
    ...
    for (const f in record) {
      ...
      AbstractQuery
          .Private
          .assembleResultEntry(query, f, record[f], config, cur, metadata[f]);
    }
    return cur;
  },
  ...
  recordsToArray: function(records, query, config, metadata) {
    const meta = AbstractQuery.Private.prepareMetadata(metadata);
    const result = [];
  ...
  paramOrder: function(sqlString, values) {
    const matches = sqlString.match(/\?\/\*\w+\*\//g);
    if (!matches && !values) {
      return [];
    } else if (!matches) {
  ...
  transformParams: function(value, type) {
    ...
    const zpad = AbstractQuery.Private.zpad;
    switch (type) {
      case 14: // DATE
        return `${
          zpad(value.getFullYear(), 4)
        }-${
          zpad(value.getMonth() + 1, 2)
        }-${
          zpad(value.getDate(), 2)
        }`;
        break;
      case 15: // TIME
        return `${
          zpad(value.getHours(), 2)
        }:${
          zpad(value.getMinutes(), 2)
        }:${
          zpad(value.getSeconds(), 2)
        }`;
        break;
      case 62: // SECONDDATE
        return `${
          ...
          stmt.exec(AbstractQuery.Private.paramOrder(sqlString, q.values)
              .filter(function(e) {
                ...
                return AbstractQuery
                    .Private
                    .transformParams(q.values[e], stmt.parameterMetadata[index].dataType);
              }), function(err, r) {
    ...
    if (!config || !config.$stream) {
      AbstractQuery.Private.executePlain(query, config, function(err, records, metadata) {
        if (err) {
          ...
          let result;
          if (records) {
            result = AbstractQuery
                .Private
                .recordsToArray(records, query, config, metadata);
          }
...
function SetQuery(
    client, subQuery1, setOperator, subQuery2, entities, leadingAlias, aliasSynonyms,
) {
...
SetQuery.prototype = function() {
  function sql() {
    ...
    if (this.orderby) {
      specifications += ` ORDER BY ${ this.orderby.map(function(o) {
        const ob = AbstractQuery.Private.serializeExpression(o.by, this.entities);
        return `${
  ...
  function execute(conn, config, callback) {
    const params = AbstractQuery
        .Private
        .normalizeExecuteCallParams(this, conn, config, callback);
    this.qconn = params.conn;
    return AbstractQuery
        .Private
        .executeAndPreFormat(this, params.config, params.callback);
  }

  function copy(e) {
    const result = new SetQuery(
        e.qconn,
        e.subQuery1,
        e.setOperator,
        e.subQuery2,
  ...
  function orderby(...args) {
    const startAlias = this.leadingAlias;
    const result = copy(this);
    result.orderby = args.map(function(o) {
      result.entities = AbstractQuery
          .Private
          .addJoinEntities(result.entities, o.$by, startAlias);
  ...
  const proto = new AbstractQuery();

  /**
     * Returns a new set query which is equivalent to the called query,
     * but with an additional ordering specification.
     * The order is specified using a number of criteria passed to the operator as arguments.
     * @param {object} criterion_i:  An object {$by: <expr>
     *                              [, $desc: true|false]
     *                              [, $nullsLast: true|false]},
     *                              where $by
     * specifies an expression (following the syntax of $where)or is a column number.
     * If $desc is set to true the order of the returned result set is in descending order,
     * otherwise it is in ascending order.
     * If $nullsLast is set to true, null values are at the end of the result set,
     * if it is false they are at the beginning. If $nullsLast is not set, the default of SQL is
     * taken (depending on ascending/descending order).
     *
     * WARNING: The columns used in <expr> must belong to the query first added
     * to the set (see example).
     * Additionally after either $order or $limit has been used set operations
     * ($union, $intersect, $except) can not be used.
     * @function
     * @memberof cds.Query.prototype
  ...
  /**
     * Executes the query and transforms the result back into
     * the structure imposed by the CDS/XSDS metadata.
     *
     * @function
     * @memberof cds.Query.prototype
     */
...
Query.prototype = function() {
  ...
  function merge(result, proj, mapping) {
    ...
    for (const f in proj) {
      ...
      if (result[f] && proj[f]) {
        const m = mapping[f].$association ?
        mapping[f].$association.$class.$_mapping :
        mapping[f];
        result[f] = merge(result[f], proj[f]===true ?
            {$all: true} :
            proj[f], m);
      }
    }
  ...
  function defaultProjectionPath(startAliases) {
    ...
    startAliases.forEach(function(a) {
      r[a] = {};
      const entity = that.entities[a].entity;
      if (entity) {
  ...
  function navigationIsLocal(projPart, entityDefPart) {
    for (const p in projPart) {
      if (!(p in entityDefPart)) {
        return false;
      }
    }
    return true;
  ...
  function processNavigation(
      res, proj, prefix, curEntity, tableAlias, aliasSynonyms, query, delimiter,
      considerFalse,
  ) {
    ...
    for (const p in proj) {
      const alias = prefix + delimiter + p;
      if (typeof proj[p] !== 'undefined' &&
        ...
        const next = curEntity.$_mapping ? curEntity.$_mapping[p] : curEntity[p];
        if (!next) {
          throw new Error(`Error in processing navigation expression: ${ p}`);
        }
        if (!next.$column) {
          AbstractQuery
              .Private
              .addAssociationJoins(entities, alias, next, curEntity, alias, tableAlias);
        ...
        const association = curEntity.$_mapping ? curEntity.$_mapping[p] : curEntity[p];
        if (association.$association && !navigationIsLocal(proj[p], association)) {
          AbstractQuery
              .Private
              .addAssociationJoins(
                  entities, alias, association, curEntity, alias, tableAlias,
              );
  ...
  function doProject(orig, result, p, startAliases, considerFalse) {
    ...
    if (typeof p === 'string') {
      ...
      let processedForLeadingAlias = false;
      for (let k = 0; k < startAliases.length; k++) {
        let startAlias = startAliases[k];
        let path;
        if (orig.entities[startAlias] && p[startAlias]) {
          ...
          if (processedForLeadingAlias) {
            continue;
          } // avoid duplicates when falling back to leading alias
        ...
        if (orig.entities[startAlias].entity) {
          processNavigation(
              ...
              path,
              startAlias,
              orig.entities[startAlias].entity,
              startAlias,
  ...
  function where(cond) {
    ...
    result.entities = AbstractQuery
        .Private
        .addJoinEntities(this.entities, cond, this.leadingAlias);
    return result;
  }
  ...
  function transformToCondition(templ, entity, mapping, entities, ref) {
      ...
      if (isNumberOrString(val)) {
        ...
        subCond[`_${
          ...
          AbstractQuery
              .Private
              .transformParams(
                  val, entity.$_metadata.sqlMetadata[mapping.$column].$type,
              )];
      } else if (val._parId) {
        subCond[`_${ prop.substring(1)}`] = [ref, val];
      } else if (Object.prototype.toString.call(val) === '[object Object]') {
        let cond = transformToCondition(val, entity, mapping, entities, ref);
        cond = cond.map(function(c) {
          ...
          }; // IS NOT NULL for each equality check to cope with SQL NULL
        });
        ...
        if (prop == '$eq') {
          ...
          throw new Error(
              `Error in matching clause: Object value of ${
                prop
              } only allowed for $eq, $neq`,
          );
        }
      } else {
        throw new Error(
            `Error in matching clause: Value of ${
              prop
            } must be a String, Number, Boolean or Date, or operator must be $eq or $neq`,
        );
      }
      return subCond;
    }
    ...
    function subCondForDateLiteral(literal, prop) {
      const subCond = {};
      let revAttributeMapping;
      for (const attribute in entity.$_metadata.revMapping) {
      ...
      AbstractQuery
          .Private
          .transformParams(
              templ[prop], entity.$_metadata.sqlMetadata[revAttributeMapping].$type,
          ),
      ];
      return subCond;
    }
    ...
    function subCondForAssocNull(nextTempl, mapping, ref) {
      const subConds = [];
      const nextMapping = mapping[prop];
      let n;
      for (const fk in nextMapping) {
    ...
    function keysNull(entity, ref) {
      let n;
      const mapping = entity.$_mapping;
      for (const k in mapping) {
        if (mapping.hasOwnProperty(k) && mapping[k].$key) {
          const reference = {
            _col: [entity, k, ref],
          };
          const r = {_null: [reference]};
          n = !n ? r : {_and: [n, r]};
        }
      }
      return n;
      ...
      if (!geoObj._geoPreOp) {
        if (geoObj.$eq) {
          ...
          if (geoObj.type) { // geoObj is a geoJSON-Obj
            const geo = require('./cds-queries-geo');
            switch (geoObj.type) {
              case 'Point':
    ...
    for (prop in templ) {
      if (!templ.hasOwnProperty(prop) || prop === '$association') {
        ...
        const sub = subCondForOperator(templ[prop], prop);
        ...
        entity.$_metadata.sqlMetadata[prop] != undefined &&
        entity.$_metadata.sqlMetadata[prop].$csType === 103
      ) {
        result.push(subCondForGeospatialType(entity, templ[prop], prop));
      } else { // nested property
        let nextEntity;
        let nextMapping;
        const foreignKeyAccess = Object.prototype
            .toString
            .call(templ[prop]) === '[object Object]' &&
                    // i.e. we follow an association but only local foreign keys are accessed
                    Object.keys(templ[prop])
                        .filter((t) => !(t in mapping[prop]))
                        .length === 0;
        ...
        const reference = foreignKeyAccess ? ref : {
          _col: [{
            $_mapping: mapping,
          },
          prop, ref,
        ...
        let subConds;
        const nextTempl = templ[prop];
        if (mapping[prop].$association && nextTempl &&
                    typeof(nextTempl.$null) !== 'undefined' &&
                    (nextTempl.$null === true || nextTempl.$null === false)
        ) { // handle the case where assocs are requested to be null
          subConds = subCondForAssocNull(nextTempl, mapping, ref);
        } else {
          subConds = transformToCondition(
              nextTempl, nextEntity, nextMapping, entities, reference,
          );
        }
        for (let k = 0; k < subConds.length; k++) {
  ...
  function matching(template) {
    ...
    const entity = this.entities[this.leadingAlias].entity;
    template = eliminateDots(template, entity.$_mapping);
    let conds;
    try {
      conds = transformToCondition(
          template,
          entity,
          entity.$_mapping, this.entities, this.leadingAlias,
      );
    } catch (e) {
      return errorQuery(e);
    }
  ...
  function summarize(by, agg, optionsAsText) {
    ...
    const result = copy(this);
    const entitiesObj = {};
    for (const e in result.entities) {
    ...
    result.aliasSynonyms = [];
    for (const p in pMap) {
      if (typeof this.aliasSynonyms[p] === 'undefined' ||
      this.aliasSynonyms[p] !== false) {
    ...
    result.entities = AbstractQuery
        .Private
        .addJoinEntities(result.entities, agg, startAlias);
    return result;
  }
  ...
  function add(addtl, bindObj) {
    ...
    for (const a in addtl) {
      ...
      result.entities = AbstractQuery
          .Private
          .addJoinEntities(result.entities, addtl[a], startAlias);
    }
    return result;
  }
      ...
      if (!mapping) {
        ...
        if (typeof mapping[v] != 'undefined' && mapping[v].$column || isFlexible) {
          ...
          valuesRec(
              ...
              mapping[v],
              isFlexible,
          ); // recursion for structured values
        }
  ...
  function values(values) {
    const result = copy(this);
    const entity = this.entities[this.leadingAlias];
    result.values = {};
    valuesRec(values,
        result.values,
        entity.entity ? entity.entity.$_mapping : null,
        entity.entity ? entity.entity.$_metadata.isFlexible : false
    );
    return result;
  }
  ...
  function copy(e) {
    const result = new Query(e.qconn, e.entities);
    result.type = e.type;
  ...
  function serializeCondition(expr, entityset, source, target) {
    if (typeof expr === 'string') {
      const matches = expr.match(/(\$SOURCE|\$TARGET)(\.\"\w+\")+/g);
      if (matches) {
        for (let k = 0; k < matches.length; k++) {
    ...
    return sql.projection.map(function(e) {
      if (typeof e === 'string') {
        ...
        if (typeof e.field != 'string') {
          return AbstractQuery
              .Private
              .serializeExpression(e.field, sql.entities) +
          (noalias ? '' : ` AS "${ e.alias }"`);
        } else {
          const fieldPath = e.field.split('.');
          if (!e.entity.$_mapping) {
            return `"${
              e.tableAlias
            }"."${
              e.entity[fieldPath[fieldPath.length - 1]].$column
            }${
                noalias ? '"' : `" AS "${
                  e.alias
                }"`
  ...
  function serializeFrom(sql) {
    return ` FROM ${ toArray(sql.entities).reduce(function(prev, e) {
      if (e.join) {
        let from = prev[prev.length - 1];
        from += ` ${ e.joinType}`;
        from += ` JOIN ${
          printTableName(e.entity.$_metadata ? e.entity.$_metadata.tableName : e.entity)
        } "${
          e.alias
        }" ON ${
      ...
      sql.groupby.clauses.map(function(e) {
        return `(${ e.map(function(e0) {
          return `"${ e0.tableAlias }"."${ e0.entity.$_mapping[e0.field].$column }"`;
            ...
            (sql.having ?
                ` HAVING ${
                  AbstractQuery.Private.serializeExpression(sql.having, sql.entities)
                }` :
                '') +
            (sql.orderby ? ` ORDER BY ${ sql.orderby.map(function(o) {
              return `${
                AbstractQuery
                    .Private
                    .serializeExpression(o.by, sql.entities)
              } ${ o.asc } ${ o.nulls}`;
            }).join(', ')}` : '');
  }
  ...
  function placeholders(sql) {
    const placeholders = [];
    if (sql.values) {
      for (const v in sql.values) {
        if (sql.values[v] && sql.values[v]._geoPreOp) {
          let argumentString = `${sql.values[v]._geoPreOp[0] }(`;
          for (let i = 1; i < sql.values[v]._geoPreOp.length; i++) {
            argumentString += typeof sql.values[v]._geoPreOp[i] === 'string' ?
            `'${ sql.values[v]._geoPreOp[i] }'` :
            sql.values[v]._geoPreOp[i];
            if (i != sql.values[v]._geoPreOp.length-1) {
              argumentString += ', ';
            }
          }
          argumentString += ')';
  ...
  function serializeInsert(sql) {
    const entity = toArray(sql.entities)[0].entity;
    return `${sql.type } INTO ${ printTableName(entity.$_metadata.tableName)
    } (${
      columnSpecs(sql.values, entity).join(', ')
    }) VALUES (${
  ...
  function serializeUpsert(sql) {
    const entity = toArray(sql.entities)[0];
    return `${sql.type } ${ printTableName(entity.entity.$_metadata.tableName)
    } (${
      columnSpecs(sql.values, entity.entity).join(', ')
    }) VALUES (${
      placeholders(sql).join(', ')
    })${
        sql.selection ?
        ` WHERE ${
          AbstractQuery
              .Private
              .serializeExpression(sql.selection, sql.entities)
              .replace(new RegExp(`"${ entity.alias }"\.`, 'g'), '')
        }` :
              ' WITH PRIMARY KEY'
    }`;
  }
  ...
  function serializeDelete(sql) {
    const entity = toArray(sql.entities)[0];
    return `${sql.type } FROM ${
      printTableName(entity.entity.$_metadata.tableName) } "${ entity.alias }" ${
             sql.selection ?
             ` WHERE ${
               AbstractQuery.Private.serializeExpression(sql.selection, sql.entities)
             }` :
             ''
    }`;
  }
  ...
  function serializeUpdate(sql) {
    ...
    return `${
      ...
      printTableName(entity.entity.$_metadata.tableName)
    } "${
      entity.alias
    }" SET ${
  ...
  function getStructureAndAddKeys(path, query) {
    ...
    const ent = query.entities[path.slice(0, path.length - 1).join('.')];
    if (path.length > 1) {
      if (!ent) {
        structure.aggregateResultSet = false;
        structure.putInArray = false;
      } else {
        const association = ent.entity.$_mapping[path[path.length - 1]].$association;
        if (association) {
          structure.putInArray = '$viaBacklink' in association || '$on' in association;
          if ('$source' in association && '$target' in association) {
            structure.putInArray = true;
            structure.aggregateResultSet = !query.entities[
                `${path.join('.')}$viaEntity`
            ].entity.$_metadata.isUnmanaged;
          }
        } else {
          structure.aggregateResultSet = false;
          structure.putInArray = false;
    ...
    const keyFilterFunction = function(proj, path, key) {
      return proj.tableAlias === path && proj.field === key.name;
    };
    const foreignKeyFilterFunction = function(
        proj, path, foreignPropertyPath, foreignPropertyName,
    ) {
      return proj.tableAlias === `${path}.${ foreignPropertyPath}` &&
       proj.field === foreignPropertyName;
    };
    ...
    if (structure.aggregateResultSet) {
      ...
      if (query.entities[joinedPath]) {
        for (const prop in query.entities[joinedPath].entity.$_metadata.keyFields) {
          const key = {name: prop};
          structure.keys.push(key);
          if (prop.indexOf('.') == -1) {
            key.generated = query.projection.filter((proj) =>
              keyFilterFunction(proj, joinedPath, key),
            ).length === 0;
            if (key.generated) {
            ...
            const foreignPropteryFullPath = prop.split('.');
            const foreignPropertyName = foreignPropteryFullPath.pop();
            const foreignPropertyPath = foreignPropteryFullPath.join('.');

            key.generated = query.projection
                .filter((proj) =>
                  foreignKeyFilterFunction(
                      proj, joinedPath, foreignPropertyPath, foreignPropertyName,
                  ),
                ).length === 0;
            if (key.generated) {
              structure.generatedKeys.push(prop);
    ...
    const children = [];
    for (let i = 0; i < query.projection.length; i++) {
      const newPath = query.projection[i].tableAlias.split('.');
      if (newPath.length > path.length) {
        ...
        if (equal && children.indexOf(newPath[path.length]) == -1) {
          ...
          structure.children
              .push(
                  getStructureAndAddKeys(path.concat(newPath[path.length]), query),
              );
        }
      }
    }
  ...
  function getObject(records, path, arrayPosition, structure) {
    let object = records[arrayPosition];
  ...
  TreeNode.prototype.add = function(element, structure, arrayPosition) {
    ...
    if (structure.aggregateResultSet) {
      ...
      for (let i = 0; i < structure.keys.length; i++) {
        const path = structure.keys[i].name.split('.');
        keys[structure.keys[i].name] = element;
        for (let j = 0; j < path.length; j++) {
          if (keys[structure.keys[i].name] &&
            typeof keys[structure.keys[i].name] == 'object' &&
            path[j] in keys[structure.keys[i].name]) {
        ...
        containsNull |= (
          (keys[structure.keys[i].name]===null) ||
        (typeof keys[structure.keys[i].name]==='undefined')
        );
      }
      ...
      if (containsNull) {
        ...
        child = this._searchChild(structure.table,
            keys,
            [0, this.children[structure.table].aggregatable.length - 1],
        );
      }
    } else {
      child = -1;
    ...
    if (typeof child == 'number') {
      this.children[structure.table].sortArray.splice(child,
          0,
          this.children[structure.table].aggregatable.length,
      );
      child = new TreeNode(structure.table, keys);
      this.children[structure.table].aggregatable.push(child);
    }
    child.arrayPosition = arrayPosition;
    for (let k = 0; k < structure.children.length; k++) {
      if (structure.children[k].table in element) {
        child.add(
            element[structure.children[k].table],
            structure.children[k],
            arrayPosition);
      }
    }
  };
  ...
  TreeNode.prototype.mergeChildren = function() {
    for (const table in this.children) {
      this.children[table] = this.children[table].aggregatable
          .concat(this.children[table].nonAggregatable);
      for (let i = 0; i < this.children[table].length; i++) {
  ...
  TreeNode.prototype.applyAll = function(records, structure) {
    const resultArray = [];
    if (structure.table in this.children) {
      for (let i = 0; i < this.children[structure.table].length; i++) {
        const child = this.children[structure.table][i];
        resultArray.push(getObject(records, [], child.arrayPosition, structure));
        for (let j = 0; j < structure.children.length; j++) {
          const childStructure = structure.children[j];
          if (childStructure.table in child.children) {
            if (childStructure.putInArray) {
              ...
              for (let k = 0; k < child.children[childStructure.table].length; k++) {
                resultArray[resultArray.length - 1][childStructure.table]
                    .push(
                        child.children[childStructure.table][k]
                            .apply(records, childStructure, [childStructure.table]),
                    );
              }
            } else {
              resultArray[resultArray.length - 1][childStructure.table] =
              child.children[childStructure.table][0]
                  .apply(records, childStructure, [childStructure.table],
                  );
            }
          }
  ...
  TreeNode.prototype.apply = function(records, structure, path) {
    const result = getObject(records, path, this.arrayPosition, structure);
    for (let i = 0; i < structure.children.length; i++) {
      const childStructure = structure.children[i];
      const childPath = path.concat(childStructure.table);
      if (childStructure.putInArray) {
        result[childStructure.table] = [];
        if (this.children[childStructure.table]) {
          for (let j = 0; j < this.children[childStructure.table].length; j++) {
            result[childStructure.table]
                .push(
                    this.children[childStructure.table][j]
                        .apply(records, childStructure, childPath),
                );
          }
        }
      } else if (typeof this.children[childStructure.table] !== 'undefined') {
        result[childStructure.table] =
        this.children[childStructure.table][0]
            .apply(records, childStructure, childPath);
      }
    }
    return result;
  ...
  /**
     ...
     *
     *  @return {TreeNode|integer} The child or an integer specifying where the child
  ...
  TreeNode.prototype._searchChild = function(table, keys, sliceIndices) {
    ...
    const middleChild = this.children[table]
        .aggregatable[
            this.children[table]
                .sortArray[
                    sliceIndices[0] + Math.floor((sliceIndices[1] - sliceIndices[0]) / 2)
                ]
        ];
  ...
  /** Returns a new query which is equivalent to the called query,
   * but which contains only those column values reachable through the specified path specification.
   * The projection ignores (i.e. overrides) the projection
  ...
  /** Returns a new query which is equivalent to the called query, but includes a
     ...
     * @param cond A Boolean expression specified as follows:
     * An expression is either <ul><li>a column reference constructed using e.f
     ...
     * <li>e.$eq(e0), if e and e0 are expressions (representing the SQL equality operator),
     * similar for the operators $gt, $lt, $ge, $le, $ne representing the SQL operators</li>
     ...
     * <li>e.$prefixOp(sqlOp, e0,...,en) if e, e0,...,en are expressions and sqlop is
     * a SQL prefix operator</li>
     * </li></ul>
     * For a full overview on all possible operators constructing a boolean expression,
     * see {@link cds.Expr}
     *
     * Note, that due to JavaScript pecularities, the association or property names
     * "name", "prototype", "length", "caller" need to be escaped by a $ sign.
  ...
  /**
     * Returns a new query which is equivalent to the called query,
     * but with an additional GROUP BY section and
     ...
     * pathSpec will lead to the GROUP BY clause of the generated query.
     * In addition, the projection is overridden with the same clause. It is possible
     * to hide some parts of the pathSpec, so that they appear in the GROUP BY clause
     * but not in the projection:for this, the pathSpec property needs be terminated
     * with the constant false (instead of true or a String literal).
     * @memberof cds.Query.prototype
     * @returns {cds.Query} a new query which performs the aggregation
     * @function
  ...
  /**
     ...
     * but with an additional ordering specification.
     * The order is specified using a number of criteria passed to the operator as arguments.
     * @param {object} criterion_i:
     * An object {$by: <expr> [, $desc: true|false] [, $nullsLast: true|false]}, where $by
     * specifies an expression (following the syntax of $where) or is a column number.
     * If $desc is set to true the order of the returned result set is in descending order,
     * otherwise it is in ascending order. If $nullsLast is set to true, null values
     * are at the end of the result set, if it is false they are at the beginning.
     * If $nullsLast is not set, the default of SQL is
     * taken (depending on ascending/descending order).
     * @memberof cds.Query.prototype
     * @returns {cds.Query} a new query which performs the aggregation
     * @function
  ...
  /**
     * Returns a new query which is equivalent to the called query,
     * but with additional calculated fields
  ...
  /**
     * Executes the query and transforms the result back into the structure imposed
     * by the CDS/XSDS metadata.
     *
     * @param {Object}  [config]
     * specifies the appearance of the result set. If this value is not set or empty,
     * a structured result set is returned, i.e. the result set is delivered as an
     * array or array-like object of rows, each of which contains an object.
     * That object contains the local fields of the entity and as
     * subobjects the requested values by association.
     * @param {boolean} [config.$flat]
     * if true, returns the data as delivered by the database as an array or
     * array-like object of rows, each of which contains an object with the
     * field (column) names as properties
     * @param {boolean} [config.$factorized]
     * if true, summarizes the result set based on the object's key(s).</br>
     * Example:
     * [{id: 1, 1_m_association: 0},
     * {id: 1, 1_m_association: 1}] -> [{id: 1, 1_m_association: [0, 1]}]
     * @function
     * @memberof cds.Query.prototype
     * @example SOHeader.$query().$project({SALESORDERID: true}).$execute({},
     * function(err, result) {});
     *      // example result:
     *      //  [{
     *      //    "SALESORDERID": "0500000486",
     ...
     *      //   }]
     *
     *      SOHeader.$query().$project({SALESORDERID: true})
     *                       .$execute({$factorized: true}, function(err, result) {});
     *      // example result:
     *      // [{"SALESORDERID":"0500000486",
     *      //      "items":[
     *      //           {"NETAMOUNT":"87"},
     ...
     *      // }]
     *
     *      result = SOHeader.$query().$project({SALESORDERID: true})
     *                                .$execute({$flat: true}, function(err, result) {});
     *      // example result:
     *      //  [{
     *      //    "SALESORDERID": "0500000486",
     *      //    "items.NETAMOUNT": "87"
...
Query.F = function() {
  ...
  const notation = {
    ...
    _prefixOp: function(args) {
      args.pop();
      const op = args[1].substring(1, args[1].length - 1);
      let a = [args[0]];
      args.shift();
      args.shift();
      a = a.concat(args);
    ...
    _preOp: function(args) {
      args.pop();
      const op = args[0].substring(1, args[0].length - 1);
      let a = [args[1]];
      args.shift();
      args.shift();
      a = a.concat(args);
    ...
    _col: function(args) {
      ...
      const fieldname = args[1].substring(1, args[1].length - 1);
      const posDot = fieldname.indexOf('.');
      if (!args[0]) {
        return `"${
          fieldname
        }"`; // for references to added fields, which do not have a qualification
      }
      let fieldspec;
      const mapping = args[0].$_mapping;
      if (!mapping) {
        fieldspec = fieldname;
      } else if (posDot < 0) {
      ...
      const entityset = args[args.length - 1];
      const prefix = (typeof via === 'string') ?
      via.split('"."').join('.') :
      getAlias(args[2], entityset);
      if (!fieldspec) {
  ...
  const fct = function(name, notation, typecheck) {
    return function() {
      const result = {
        notation: notation,
      };
      ...
      if (name === '_col') {
        ...
        const fieldPrefix = field.substring(0, field.indexOf('.')) || field;
        if (e) {
          const mapping = e.$_mapping;
          if (!mapping[fieldPrefix]) {
            // eslint-disable-next-line no-throw-literal
            throw (
              `Error during query construction: Field or association ${
                fieldPrefix
              } does not exist in ${
                e.$_metadata.entityName
              }`);
          }
          if (mapping[fieldPrefix].$association) {
            const targetEntity = mapping[fieldPrefix].$association.$class;
            if (targetEntity.$_mapping) {
              for (const f in targetEntity.$_mapping) {
                const createColFct = function(ff, prefix) {
            ...
            for (const fk in mapping[fieldPrefix]) {
              if (fk.substring(0, 1) === '$') {
                continue;
              }
              const createColFct = function(ff) {
                Object.defineProperty(result, ff, {
                  get: function() {
                    return Query.F.$col({
  ...
  return {
    ...
    /**
     * constructs an expression to be used in $where,
     * $addFields, or $order representing the equality of two expressions
     * @memberof cds.Expr
     * @function
    ...
    /** constructs an expression to be used in $where, $addFields,
     * or $order representing the greater-than relation between two expressions
     * @memberof cds.Expr
     * @function
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing the less-than relation between two expressions
    * @memberof cds.Expr
    * @function
    ...
    */
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing the not-equal relation between two expressions
     * @memberof cds.Expr
     * @function
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing the less-than-or-equal relation between two expressions
    * @memberof cds.Expr
    * @function
    ...
    */
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing the greater-than-or-equal relation between two expressions
         * @memberof cds.Expr
         * @function
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing the addition of two expressions
         * @memberof cds.Expr
         * @function
    ...
    /** constructs an expression to be used in $where, $addFields,
     * or $order representing the subtraction of two expressions
         * @memberof cds.Expr
         * @function
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing the multiplication of two expressions
         * @memberof cds.Expr
         * @function
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing the division of two expressions
         * @memberof cds.Expr
         * @function
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing that an expression is equal to null.
     * If an additional argument "false" is given the expression states
         * that the first expression is not null.
         * @memberof cds.Expr
         * @function
         * @example soHeader.$query()
         * .$where(soHeader.BILLINGSTATUS.$null())
         * --> generates: ... WHERE "t0"."BILLINGSTATUS" IS NULL
         * @example soHeader.$query().$where(soHeader.BILLINGSTATUS.$null(false))
         * --> generates: ... WHERE "t0"."BILLINGSTATUS" IS NOT NULL
         */
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing that the first
     * expression argument is NOT LIKE the second expression argument
         * @memberof cds.Expr
         * @function
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing that the first
     * expression argument is LIKE the second expression argument
         * @memberof cds.Expr
         * @function
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing that all boolean expression arguments should hold
         * @memberof cds.Expr
         * @function
         * @example soHeader.$query()
         * .$where(soHeader.NETAMOUNT.$lt(10).$and(soHeader.DELIVERYSTATUS.$eq("I")))
         */
    $and: fct('_and', notation['_and']),

    /** constructs an expression to be used in $where,
     * $addFields, or $order representing that one of boolean expression arguments should hold
         * @memberof cds.Expr
         * @function
         * @example soHeader.$query()
         * .$where(soHeader.NETAMOUNT.$lt(10).$or(soHeader.DELIVERYSTATUS.$eq("I")))
         */
    $or: fct('_or', notation['_or']),

    /** constructs an expression to be used in $where,
     * $addFields, or $order representing that the given boolean expression should not hold
         * @memberof cds.Expr
         * @function
         *
         * @example soHeader.$query()
         * .$where(soHeader.NETAMOUNT.$lt(10).$and(soHeader.DELIVERYSTATUS.$eq("I").$not()))
         * --> generates ... WHERE ("t0"."NETAMOUNT"<10) AND ( NOT "t0"."DELIVERYSTATUS"='I')
         */
    $not: fct('_not', notation['_not']),

    /** constructs an expression to be used in $where,
     * $addFields, or $order representing the average function application on an expression
         * @memberof cds.Expr
         * @function
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing the maximum function application on an expression
         * @memberof cds.Expr
         * @function
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing the minimum function application on an expression
         * @memberof cds.Expr
         * @function
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing the sum function application between two expressions
         * @memberof cds.Expr
         * @function
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing the count function application between two expressions
         * @memberof cds.Expr
         * @function
    ...
    /** constructs an expression to be used in $where,
     * $addFields, or $order representing an infix operation specified as first argument
         * @memberof cds.Expr
         * @function
         * @example soHeader.$query()
         * .$where(soHeader.CURRENCY.$infixOp("LIKE", "%UR"))
         * --> generated: ...WHERE "t0"."CURRENCY" LIKE '%UR'",
         */
    $infixOp: fct('_infixOp', notation['_infixOp']),

    /** constructs an expression to be used in $where,
     * $addFields, or $order representing a prefix operation specified as first argument
         * @memberof cds.Expr
         * @function
...
Query.addExpressionFunctions = function(entity, obj, prefix) {
  function defineFct(onObj, f, prefix) {
    ...
    if (typeof onObj[f] !== 'undefined') {
      // no shortcut because of built-in properties 'name', 'length', 'caller', 'prototype'
      return;
    }
  ...
  const mapping = entity.$_mapping || entity;
